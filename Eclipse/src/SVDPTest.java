...
public class SVDPTest {
	...
	public double LRATE_UB_INITIAL = 1e-3;
	public double LRATE_MB_INITIAL = 2.213e-3;
	public double K_MW = 1.225e-2;
	public double K_UB = 3.217e-4;
	public double K_MB = 2.754e-3;
	public double K_UF = 0.01;
	public double K_MF = 0.01;
	public double INIT_BASE = 10;

	// Tested variables.
	public double LRATE_MW_BEST;
	...
	public SVDPTest(int numFeatures) {
	...
	public SVDPTest(int numFeatures, int testParam) {
		this.NUM_FEATURES = numFeatures;
	...
	public SVDPTest(int numFeatures, double LRATE_BIAS, double LRATE_FEATURES,
	...
	private void tweakOneParam() {
		int randChoice = (int) (Math.random() * 8);

		if (randChoice == 0)
			LRATE_MW_INITIAL = tweakParam(LRATE_MW_INITIAL);
		else if (randChoice == 1)
			LRATE_UB_INITIAL = tweakParam(LRATE_UB_INITIAL);
		else if (randChoice == 2)
			LRATE_MB_INITIAL = tweakParam(LRATE_MB_INITIAL);
		else if (randChoice == 3)
			K_MW = tweakParamSlightly(K_MW);
		else if (randChoice == 4)
			K_UB = tweakParamALot(K_UB);
		else if (randChoice == 5)
			K_MB = tweakParamALot(K_MB);
		else if (randChoice == 6)
			K_UF = tweakParamSlightly(K_UF);
		else if (randChoice == 7)
			K_MF = tweakParamSlightly(K_MF);
	}

	private String genParamStr() {
		String str = "";
		str += "1 " + LRATE_MW_INITIAL + "\n";
		str += "2 " + LRATE_UB_INITIAL + "\n";
		str += "3 " + LRATE_MB_INITIAL + "\n";
		str += "4 " + K_MW + "\n";
		str += "5 " + K_UB + "\n";
		str += "6 " + K_MB + "\n";
		str += "7 " + K_UF + "\n";
	...
	private void saveBestParams() {
		LRATE_MW_BEST = LRATE_MW_INITIAL;
		...
		K_UF_BEST = K_UF;
		K_MF_BEST = K_MF;
		INIT_BASE_BEST = INIT_BASE;
	}

	private void restoreBestParams() {
		LRATE_MW_INITIAL = LRATE_MW_BEST;
		...
		while (true) {
			...
			for (int i = 1; true; i++) {
				...
				if (rmseDiff < MIN_ERROR_DIFF) {
					...
					System.out.println(String.format("Tried probe RMSE %.5f (%.2f%%)",
							rmse, (1 - rmse / CINEMATCH_BASELINE) * 100));
					System.out.println(String.format("Best probe RMSE %.5f (%.2f%%)",
		...
		for (int j = 0; j < numPoints; j++) {
			...
			for (int k = 0; k < NUM_FEATURES; k++) {
				...
				movieFeatures[movie][k] += LRATE_MF
						* (err * (uf + norm[user] * sum_mw[user][k]) - K_MF * mf);
	...
	public static void main(String[] args) throws NumberFormatException,
		...
		SVDPTest trainer;
		if (args.length == 1) {
			trainer = new SVDPTest(Integer.parseInt(args[0]));
