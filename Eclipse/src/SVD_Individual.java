...
public class SVD_Individual {
	...
	public SVD_Individual(int numFeatures) {
	...
	public SVD_Individual(int numFeatures, int testParam) {
		this.NUM_FEATURES = numFeatures;
	...
	public SVD_Individual(int numFeatures, double LRATE_BIAS,
			double LRATE_FEATURES, double LRATE_MW, double K_BIAS, double K_FEATURES,
			double K_MW) {
	...
	public double trainWithNumPoints(int numPoints) throws IOException {
		...
		for (int k = 0; k < NUM_FEATURES; k++) {
			for (int j = 0; j < numPoints; j++) {
				...
				// Train biases.
				userBias[user] += LRATE_USER_BIAS
						* (err - K_USER_BIAS * userBias[user]);
	...
	private void generateOutput() throws IOException {
		FileWriter fstream = new FileWriter(
				"SVD_Individual_1234_with_probe_training");
		...
		fstream = new FileWriter("SVD_Individual_5_with_probe_training");
	...
	private void generateProbeOutput() throws IOException {
		FileWriter fstream = new FileWriter("SVD_Individual_123_no_probe_training");
		BufferedWriter out = new BufferedWriter(fstream);
		int user;
		short movie, date;
		double predictedRating;
		...
		fstream = new FileWriter("SVD_Individual_4_no_probe_training");
		out = new BufferedWriter(fstream);
		// Test the model in probe set.
		for (int j = 0; j < probeData.length; j++) {
			user = probeData[j][0];
	...
	public static void main(String[] args) throws NumberFormatException,
		...
		SVD_Individual trainer;
		if (args.length == 1) {
			trainer = new SVD_Individual(Integer.parseInt(args[0]));
